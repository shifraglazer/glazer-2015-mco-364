...
public class SnakeBody {
	...
	public void getLonger() {
		...
		case "Up": {
			...
			cell = new SnakeCell((snake.getLast().getX()), snake.getLast()
					.getY() + SIZE, SIZE, SIZE, ARCSIZE);
			break;
		}
		case "Down": {
			cell = new SnakeCell(snake.getLast().getX(), snake.getLast().getY()
					- SIZE, SIZE, SIZE, ARCSIZE);
			break;
		}
		case "Right": {
			cell = new SnakeCell((snake.getLast().getX() - SIZE), snake
					.getLast().getY(), SIZE, SIZE, ARCSIZE);
			break;
		}
		case "Left": {
			cell = new SnakeCell((snake.getLast().getX() + SIZE), snake
	...
	public void moveForward(String direction) {
		...
		case "Up": {
			...
			cell = new SnakeCell((snake.getFirst().getX()), snake.getFirst()
					.getY() - SIZE, SIZE, SIZE, ARCSIZE);
			break;
		}
		case "Down": {
		...
		case "Right": {
			cell = new SnakeCell((snake.getFirst().getX() + SIZE), snake
					.getFirst().getY(), SIZE, SIZE, ARCSIZE);
			break;
		}
		case "Left": {
			cell = new SnakeCell((snake.getFirst().getX() - SIZE), snake
	...
	public boolean bangWall() {
		...
		if (first.getX() < 0 || first.getX() > width || first.getY() < 0
				|| first.getY() > height) {
